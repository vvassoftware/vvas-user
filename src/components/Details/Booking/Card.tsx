...
export default function Card({
  ...
  const currentDateCard = formatWithUserTimeZone(
    true,
    `${selectedDay} ${startTime}`
  );

  const isBeforeDay = currentDateCard.isBefore(today, "hour");
  ...
  return (
    <div
      className={`${
        listBoats?.length === 0 || isBeforeDay
          ? "bg-red-500"
          : status
      ...
      onClick={
        listBoats?.length === 0 || isBeforeDay ? () => {} : onClick
      }
    >
      <div
        className={`text-xs font-bold ${
          status || isBeforeDay ? "text-white" : "text-darkBlue"
        }`}
      >
        {startTime.split(" ")[0]}
      ...
      <div className="flex items-center gap-x-1">
        ...
        >
          <path
            ...
            fill={`${status || isBeforeDay ? "#FFFFFF" : "#1B4965"}`}
          />
        </svg>
        ...
        >
          {isBeforeDay ? "N/A" : listBoats?.length}
        </span>
      </div>
      <span
        className={`block ${
